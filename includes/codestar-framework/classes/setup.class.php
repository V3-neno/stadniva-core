...
if ( ! class_exists( 'CSF' ) ) {
  class CSF {
    ...
    public static function add_admin_enqueue_scripts() {
      ...
      // Getting google map key from admin panel option
      $key = get_option( 'admin_map_key' );

      // framework core scripts
      wp_enqueue_script( 'csf-plugins', CSF::include_plugin_url( 'assets/js/csf-plugins'. $min .'.js' ), array(), '1.0.0', true );
      wp_register_script( 'csf', CSF::include_plugin_url( 'assets/js/csf'. $min .'.js' ), array( 'csf-plugins' ), '1.0.0', true );
      wp_enqueue_script( 'csf' );
      wp_localize_script( 'csf' , 'map_key' , json_encode( $key ) );
